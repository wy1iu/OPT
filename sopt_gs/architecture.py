...
class VGG():
    ...
    def get_rotation(self, shape):
        init = tf.constant_initializer(np.eye(shape[0]), dtype=tf.float32)
    ...
    def tf_gram_schmidt(self, vectors):
    ...
    def _conv_layer_first(self, bottom, ksize, n_filt, is_training, name, stride=1, 
        ...
        with tf.variable_scope(name) as scope:
            ...
            orth_rotate = self.tf_gram_schmidt(rotate)
    ...
    def _conv_layer_sparse(self, bottom, ksize, n_filt, is_training, idx, name, stride=1, 
        pad='SAME', relu=False, reg=True,  bn=True):

        with tf.variable_scope(name) as scope:
            n_input = bottom.get_shape().as_list()[3]
            shape1 = [ksize*ksize*n_input, n_filt]
            shape2 = [ksize*ksize*10, ksize*ksize*10]
            shape = [ksize, ksize, n_input, n_filt]
            print("shape of filter %s: %s" % (name, str(shape)))
            ...
            fix_s = tf.gather(fix_filt, idx)
            updates = tf.matmul(orth_rotate,fix_s)
            filt = tf.tensor_scatter_nd_update(fix_filt, idx[:, tf.newaxis], updates)
            self.assign_ops.append(fix_filt.assign(filt))
    ...
    def build(self, rgb, n_class, is_training, idx_list):
        self.wd = 5e-4
        self.assign_ops = []
        ...
        feat = self._conv_layer_first(feat, ksize, n_out, is_training, name="conv1_0", bn=True, relu=True, pad='SAME', reg=True)
        feat = self._conv_layer_sparse(feat, ksize, n_out, is_training, idx_list[0], name="sparse_conv1_1", bn=True, relu=True, pad='SAME', reg=True)
        feat = self._max_pool(feat, 'pool1')

        # 16X16
        n_out = 64
        for i in range(n_layer):
            feat = self._conv_layer_sparse(feat, ksize, n_out, is_training, idx_list[1+i], name="sparse_conv2_" + str(i), bn=True, relu=True,
                                    pad='SAME',  reg=True)
        ...
        n_out = 64
        for i in range(n_layer):
            feat = self._conv_layer_sparse(feat, ksize, n_out, is_training, idx_list[3+i], name="sparse_conv3_" + str(i), bn=True, relu=True,
                                    pad='SAME',  reg=True)
        feat = self._max_pool(feat, 'pool3')

        self.fc6 = self._conv_layer_sparse(feat, 4, 64, is_training, idx_list[5], "sparse_fc6", bn=False, relu=False, pad='VALID',
                                    reg=True)
