...
from architecture import VGG, OGD
import pickle
...
def train(base_lr, batch_sz, gpu_no):
    ...
    if len(reg_loss_list) != 0:
        ...
        loss_op += 0.1 * reg_loss

    all_vars = tf.trainable_variables()
    rota_vars = [var for var in all_vars if 'rotation' in var.name]
    train_vars = [var for var in all_vars if 'fix' not in var.name and 'rotation' not in var.name]
    rota_op = OGD(rota_lr_).minimize(fit_loss, var_list=rota_vars)
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_epoch):
            ...
            fit, reg, acc,  _, _  = sess.run([fit_loss, reg_loss, acc_op, update_op,rota_op],{lr_: lr, rota_lr_: rota_lr, is_training: True, images: tr_images, labels: tr_labels})
