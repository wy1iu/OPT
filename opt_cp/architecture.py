...
class VGG():
    ...
    def cayley(self, data):
        ...
        upper =  tf.matrix_band_part(R, 0, -1)
        skew = upper + tf.transpose(-1*upper)
        Q = tf.matmul(tf.eye(r)+skew,tf.matrix_inverse(tf.eye(r)-skew))
    ...
    def _conv_layer(self, bottom, ksize, n_filt, is_training, name, stride=1, 
        ...
        with tf.variable_scope(name) as scope:
            ...
            orth_rotate = self.cayley(rotate)
    ...
    def build(self, rgb, n_class, is_training):
        ...
        self.fc6 = self._conv_layer(feat, 4, 128, is_training, "fc6", bn=False, relu=False, pad='VALID',
                                    reg=True)
