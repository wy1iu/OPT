...
class VGG():
    ...
    def add_orthogonal_constraint(self, filt, n_filt):
        inner_pro = tf.matmul(filt, tf.transpose(filt))
        loss = tf.sqrt(tf.reduce_sum((inner_pro - tf.eye(n_filt))**2))
        tf.add_to_collection('orth_constraint', loss)

    def get_filter_norm(self, filt):
    ...
    def _conv_layer(self, bottom, ksize, n_filt, is_training, name, stride=1, 
        ...
        with tf.variable_scope(name) as scope:
            ...
            self.add_orthogonal_constraint(rotate, ksize*ksize*n_input)
